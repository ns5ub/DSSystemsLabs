...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 13 17:10:42 2022
...
cur.execute("""CREATE TABLE IF NOT EXISTS users(
   ...
   lname TEXT,
   gender TEXT);
""") # Not Exists prevents error on rerunning code
...
user = ('00002', 'Lois', 'Lane', 'Female') # python tuple
cur.execute("INSERT INTO users VALUES(?, ?, ?, ?);", user) #prevents against injection attacks using ?, ?

more_users = [('00003', 'Peter', 'Parker', 'Male'), ('00004', 'Bruce', 'Wayne', 'male')] #multiple tuples at once in a list
...
print("SELECT ONE USER")
print(one_result)

print()
cur.execute("SELECT * FROM users;")
three_results = cur.fetchmany(3) #first 3 results
print("SELECT FIRST 3 USERS")
print(three_results)

print()
cur.execute("SELECT * FROM users;")
all_results = cur.fetchall() #print out query wholesale
print(all_results)


#FIND/DELETE DATA
cur.execute("DELETE FROM users WHERE lname='Parker';") #delete is based on where
conn.commit()

print()
print("DELETING PARKER (just like <spoilers>)")
#Empty since been Deleted
